#!/usr/bin/env node
/**
 * CLI: run a single stablecoin or yield agent pass (read-only research).
 *
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--type" && next) type = next as AgentType;
      ...
      console.log(`Usage: run-agent.ts --type stablecoin|yield --asset ${listWatchedAssetSymbols().join("|")} [--agent-id id]`);
      process.exit(0);
    }
  }
...
async function main() {
  ...
  if (!type || (type !== "stablecoin" && type !== "yield")) {
    console.error("Missing or invalid --type (stablecoin | yield)");
    process.exit(1);
  }
  if (!assetSymbol) {
