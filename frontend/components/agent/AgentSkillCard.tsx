...
function CopyButton({ label, text }: { label: string; text: string }) {
  ...
  return (
    <button
      type="button"
      onClick={copy}
      className="inline-flex items-center gap-1.5 rounded-lg border border-ink-700 bg-ink-900/60 px-3 py-1.5 text-xs font-medium text-ink-100 transition-colors hover:text-ink-50"
    >
  ...
  return (
    <details className="group rounded-lg border border-ink-800/60 bg-ink-900/30">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-4 py-3 text-sm font-medium text-ink-100">
        {heading}
        <ChevronDown className="h-4 w-4 shrink-0 text-ink-300 transition-transform group-open:rotate-180" />
      </summary>
  ...
  return (
    <section id="agent-skill" className="scroll-mt-24 space-y-4">
      <SectionHeading
        ...
        subtitle="Machine-readable protocol knowledge for agents"
      />
      ...
      <Card className="space-y-4">
        ...
        <div className="space-y-2">
          <p className="text-xs font-medium uppercase tracking-wider text-ink-300">Facts</p>
          <TableShell>
            <Table>
              <TBody>
                {skill.facts.map((f) => (
                  <TR key={f.key}>
                    <TD className="w-1/3 font-mono text-xs text-ink-300">{f.key}</TD>
        ...
        <div className="space-y-2">
          <p className="text-xs font-medium uppercase tracking-wider text-ink-300">Sections</p>
          <div className="space-y-2">
        ...
        <div className="space-y-2">
          <p className="text-xs font-medium uppercase tracking-wider text-ink-300">Actions</p>
          <TableShell>
            <Table>
              <THead>
                <TR>
        ...
        {skill.glossary && skill.glossary.length > 0 && (
          <div className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-wider text-ink-300">Glossary</p>
            <dl className="space-y-2">
              {skill.glossary.map((g) => (
                <div key={g.term} className={cn("rounded-lg bg-ink-900/40 px-3 py-2")}>
                  ...
                  <dd className="mt-0.5 text-sm text-ink-300">{g.definition}</dd>
                </div>
              ))}
            </dl>
          </div>
